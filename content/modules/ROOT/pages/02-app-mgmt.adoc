...
NOTE: This module intentionally switches between CLI and Console as you go - the goal is to show both paths, since ops engineers reach for whichever is available or more convenient in the moment. Deploying here starts in the Console so you can see what gets created; most later steps perform the action via CLI and then show the same result in the Console for visual confirmation.
...
TIP: Once you're comfortable with the Console flow above, the same deployment collapses into a single CLI command: `oc new-app --name=weathernow quay.io/openshift-workshop-applications/weathernow:latest`. Good to have both in your toolkit - the Console for visibility into what's being created, the CLI for speed.
...
NOTE: OpenShift also supports `passthrough` and `reencrypt` termination, where the pod itself serves TLS and the router forwards or re-encrypts it. WeatherNow only serves plain HTTP, so edge is the only option that applies here - in a real application capable of serving its own TLS, passthrough or reencrypt might be preferred instead.
...
TIP: If you don't see the **Workloads** menu, switch to the **Core Platform** perspective using the dropdown in the top left.
...
The Deploy Image workflow wired weathernow to an ImageStream trigger that resets the image back to `weathernow:latest` any time it drifts - useful in production, but it would silently undo our simulated bad deploy. Remove it for this exercise:

[source,bash,role="execute"]
----
oc set triggers deployment/weathernow -n app-management --from-image=app-management/weathernow:latest --containers=weathernow --remove
----

Now simulate a bad deploy by changing the image:
...
`ubi9/nginx-124` is an S2I builder image, not something meant to run standalone, so the new pods will crash-loop - a realistic "bad deploy." Annotate the deployment so the revision history records why this happened:

[source,bash,role="execute"]
----
oc annotate deployment/weathernow kubernetes.io/change-cause="Bad image update for rollback demo" -n app-management --overwrite
----
...
You'll see multiple revisions, each with a `CHANGE-CAUSE` showing why it was created. Every time you change the deployment, a new revision is created.
...
Each ReplicaSet represents a revision. Because the new pods are crash-looping, the rollout is stuck partway: the old revision stays up instead of scaling to `0 of 0`, and the new one never reaches ready. OpenShift keeps the old ReplicaSet around so it can roll back instantly.

Roll back to the previous working version, then annotate again:
...
[source,bash,role="execute"]
----
oc annotate deployment/weathernow kubernetes.io/change-cause="Rolled back to previous known-good image" -n app-management --overwrite
----
...
You should see the WeatherNow image resolved through the internal registry (OpenShift pulls the external image and stores it locally via the ImageStream):

----
image-registry.openshift-image-registry.svc:5000/app-management/weathernow@sha256:...
----
...
This matters to you because a misconfigured PDB is one of the most common causes of stalled cluster upgrades - the upgrade process drains nodes one at a time, and a single bad PDB can block the entire upgrade indefinitely.
...
* **Rollback** - `oc rollout undo` reverts a bad deployment in seconds
