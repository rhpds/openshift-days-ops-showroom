...
The Deploy Image workflow you used earlier wired weathernow to an ImageStream trigger, which automatically resets the container image back to the `weathernow:latest` tag any time it drifts. That's the right behavior in production, but it would silently undo our simulated bad deploy before you ever saw it. Remove it for this exercise:
...
`ubi9/nginx-124` is an S2I builder image, not something meant to run standalone, so the new pods will crash-loop - a realistic "bad deploy." Annotate the deployment now so the revision history records why this happened. Doing this after the image change (not before) matters: annotating a deployment stamps the change-cause onto whichever revision is currently active, so annotating first would have mislabeled the *previous, good* revision instead:
...
Each ReplicaSet represents a revision. Because the new pods are crash-looping, the rollout is stuck partway: the previous (good) revision still shows pods running instead of scaling down to `0 of 0`, while the new (bad) revision shows pods that exist but never become ready. OpenShift won't finish cutting over until the new pods pass their health checks - and it keeps the old ReplicaSet around so it can roll back instantly.

Roll back to the previous working version, then annotate again - same reasoning as before, the annotation should follow the change, not precede it:
