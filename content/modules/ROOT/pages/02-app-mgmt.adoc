...
+
image::app-mgmt-deploy-menu.png[Add menu showing Container images option]
...
+
image::app-mgmt-uncheck-route.png[Create a route checkbox unchecked]
* Click **Create**
+
image::app-mgmt-deploy-image.png[Deploy Image form with weathernow image validated]
...
image::diagrams/02_service.png[OpenShift Service Load Balancing, 500, align=center]
...
image::app-mgmt-deploy-no-route.png[Topology view showing weathernow with no route]
...
image::app-mgmt-pod-details.png[Pod detail page showing Overview with Events and resource usage]
...
image::diagrams/02_route.png[OpenShift Route - Service - Pods, 500, align=center]
...
image::app-mgmt-deploy-topology.png[Topology view showing weathernow application with route]

Click the route URL to open the WeatherNow app in your browser:

image::app-mgmt-weathernow-app.png[WeatherNow application showing weather data]
...
image::app-mgmt-scaling-pods.png[Pod list showing 3 weathernow pods after scaling]
...
Start watching pods, then delete one - you'll see every status transition in real-time:

[source,bash,role="execute"]
----
oc get pods -n app-management -w &
sleep 2 && oc delete pod $(oc get pods -n app-management -o jsonpath='{.items[0].metadata.name}') -n app-management --wait=false
----

Watch the output - you'll see the deleted pod transition to `Terminating` while a new pod appears as `Pending` → `ContainerCreating` → `Running`. The total stays at 3 replicas throughout.

Press `Ctrl+C` when you see the replacement pod reach `Running`.

TIP: You can also watch this in the **OCP Console** at **Workloads -> Pods** (project: `app-management`). The console shows pod status transitions in real-time with colour-coded indicators.
...
Watch the rollout - it will stall because the new pods never become ready:
...
NOTE: If you see `Waiting for deployment "weathernow" rollout to finish: 0 of 1 updated replicas are available...` hanging, press **Ctrl+C** to exit and continue.
...
image::broken-probe-pods.png[Pod list showing 0/1 Running pod with failed readiness probe]
...
image::broken-probe-events.png[Events tab showing readiness probe failure messages]
...
image::deployment-actions-menu.png[Deployment actions menu showing Edit Health Checks option]
...
image::edit-health-checks.png[Edit Health Checks form showing configured readiness and liveness probes]
...
image::app-mgmt-replicasets-history.png[ReplicaSets tab showing deployment revision history with active and scaled-down replicas]
...
NOTE: If the **Ingress & Load Balancing** module is part of your workshop, **skip this cleanup** - it reuses the `weathernow` application in the `app-management` namespace.

[source,bash,role="execute"]
----
oc delete namespace app-management --ignore-not-found --wait=false
