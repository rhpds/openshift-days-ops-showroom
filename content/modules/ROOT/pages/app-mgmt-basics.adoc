...
Add both probes to the deployment. We'll use `tcpSocket` probes that check whether the container is listening on port 8080:
...
NOTE: Probes support multiple types: `httpGet` (checks an HTTP endpoint), `tcpSocket` (checks if a port is listening), and `exec` (runs a command inside the container). Use whichever makes sense for your application.
...
TIP: In the **OCP Console**, navigate to **Workloads -> Deployments -> weathernow** and select **Actions -> Edit Health Checks**. The form visually explains the difference between liveness, readiness, and startup probes — and lets you configure them without writing YAML. This is a great way to understand the probe types.
