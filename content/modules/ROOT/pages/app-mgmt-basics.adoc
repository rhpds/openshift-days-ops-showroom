...
You should see pods showing `0/1 Running` — the container is running, but the readiness probe is failing so OpenShift won't send traffic to them. In the **OCP Console**, go to **Workloads -> Pods** (project: `app-management`), click any of the new pods, then the **Events** tab. You'll see repeated `Readiness probe failed: dial tcp ... connect: connection refused` messages.
...
TIP: In the **OCP Console**, navigate to **Workloads -> Deployments -> weathernow** and select **Actions -> Edit Health Checks**. The form visually shows the difference between liveness, readiness, and startup probes — and lets you configure them without writing JSON patches.
