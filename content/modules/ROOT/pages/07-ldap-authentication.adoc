...
[source,bash,role="execute"]
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/deploy-glauth-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/main/support/07-ldap-authentication/deploy-glauth-ldap.sh; then
  bash /tmp/deploy-glauth-ldap.sh
else
  echo "ERROR: Could not reach GitHub to download deploy-glauth-ldap.sh. Check your network connection and re-run this command - GitHub may be experiencing a temporary outage."
fi
----
...
[source,bash,role="execute"]
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/configure-oauth-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/07-ldap-authentication/configure-oauth-ldap.sh; then
  bash /tmp/configure-oauth-ldap.sh
else
  echo "ERROR: Could not reach GitHub to download configure-oauth-ldap.sh. Check your network connection and re-run this command - GitHub may be experiencing a temporary outage."
fi
----
...
----
oc login -u {openshift_cluster_admin_username} -p {openshift_cluster_admin_password} --insecure-skip-tls-verify
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/cleanup-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/cleanup-scripts/cleanup-ldap.sh; then
  bash /tmp/cleanup-ldap.sh
else
  echo "ERROR: Could not reach GitHub to download cleanup-ldap.sh. Check your network connection and re-run this command - GitHub may be experiencing a temporary outage."
fi
----
