...
TIP: For the full list of supported identity providers, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/understanding-identity-provider[Understanding identity providers, window=blank].
...
This module deploys a lightweight LDAP server inside your cluster (no external directory required) with the following groups:
...
1. **LDAP server** - a lightweight in-cluster directory pre-loaded with the users and groups from Step 2
2. **Secret** containing the LDAP bind password
3. **ConfigMap** containing the LDAP server CA certificate
4. **OAuth configuration** defining the LDAP identity provider

**Deploy the LDAP server:**

[source,bash,subs="+attributes",role="execute"]
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/deploy-glauth-ldap.sh {support_base_url}/07-ldap-authentication/deploy-glauth-ldap.sh; then
  bash /tmp/deploy-glauth-ldap.sh
elif curl -fsSL --retry 3 --retry-delay 2 -o /tmp/deploy-glauth-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/07-ldap-authentication/deploy-glauth-ldap.sh; then
  bash /tmp/deploy-glauth-ldap.sh
else
  echo "ERROR: Could not download deploy-glauth-ldap.sh from the Showroom instance or GitHub. Check your network connection and re-run this command."
fi
----

This deploys link:https://github.com/glauth/glauth[glauth, window=blank] - a lightweight, single-container LDAP server - into its own `glauth` namespace, pre-loaded with the users and groups from Step 2. Unlike a real enterprise directory, it runs entirely inside your cluster, so this module has no dependency on an external service being reachable. It also creates a passthrough Route so LDAP clients running outside the cluster (like the `oc adm groups sync` command later in this module) can reach it too.
...
**Extract the CA certificate from the LDAP server:**

[source,bash,role="execute"]
----
mkdir -p $HOME/support
oc extract secret/glauth-tls -n glauth --keys=tls.crt --to=- > $HOME/support/ca.crt
----

NOTE: `deploy-glauth-ldap.sh` generated a self-signed TLS certificate for the LDAP server and stored it in the `glauth-tls` Secret. Here we pull that same certificate out so OpenShift can validate the LDAP server's TLS connection. In an environment with a real enterprise directory, you'd get this CA certificate from your LDAP provider instead.
...
[source,bash,subs="+attributes",role="execute"]
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/configure-oauth-ldap.sh {support_base_url}/07-ldap-authentication/configure-oauth-ldap.sh; then
  bash /tmp/configure-oauth-ldap.sh
elif curl -fsSL --retry 3 --retry-delay 2 -o /tmp/configure-oauth-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/07-ldap-authentication/configure-oauth-ldap.sh; then
  bash /tmp/configure-oauth-ldap.sh
else
  echo "ERROR: Could not download configure-oauth-ldap.sh from the Showroom instance or GitHub. Check your network connection and re-run this command."
fi
----

This script creates an OAuth configuration that adds the LDAP identity provider alongside the existing HTPasswd provider. It configures: the bind DN (`cn=openshiftworkshop,...`), the LDAP URL (`ldaps://glauth.glauth.svc.cluster.local`) with a `memberOf` filter restricting login to the `ose-user` group, attribute mappings (`dn` -> ID, `mail` -> email, `cn` -> display name, `uid` -> username), and references the CA cert and bind password you created above.
...
For detailed LDAP configuration, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/configuring-identity-providers#configuring-ldap-identity-provider[Configuring an LDAP identity provider, window=blank].
...
NOTE: Unlike the OAuth login flow (which runs entirely inside the cluster), `oc adm groups sync` connects to the LDAP server directly from wherever you run the command. It can't resolve the in-cluster service address, so this step uses the external route that `deploy-glauth-ldap.sh` created for glauth instead.

**Create the group sync configuration:**

[source,bash,role="execute"]
----
GLAUTH_ROUTE=$(oc get route glauth -n glauth -o jsonpath='{.spec.host}')

cat <<EOF > $HOME/support/groupsync.yaml
kind: LDAPSyncConfig
apiVersion: v1
url: ldaps://$GLAUTH_ROUTE:443
bindDN: cn=openshiftworkshop,ou=svcaccts,ou=users,dc=openshiftworkshop,dc=com
bindPassword: b1ndP^ssword
ca: $HOME/support/ca.crt
rfc2307:
  groupsQuery:
    baseDN: ou=users,dc=openshiftworkshop,dc=com
    derefAliases: never
    filter: '(|(cn=ose-*))'
...
This configuration searches for LDAP groups matching `ose-*`, creates matching OpenShift Groups, and populates membership from LDAP. It reuses the `ca.crt` you extracted in Step 3 to validate the LDAP server's TLS certificate.
...
For more on roles, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/using-rbac[Using RBAC to define and apply permissions, window=blank].
...
**Verify in the console.** Navigate to **User Management -> RoleBindings**, select project `app-dev`, and filter by kind and select **Namespace RoleBindings**:
...
For complete implementation details, see the link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/ldap-syncing#ldap-syncing-running-all-ldap-sync_configuring-ldap-syncing[official documentation on automating LDAP sync, window=blank].
...
* **Authentication and Authorization Guide:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/index[Official documentation, window=blank]
* **Identity Providers Overview:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/understanding-identity-provider[Understanding identity providers, window=blank]
* **LDAP Configuration:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/configuring-identity-providers#configuring-ldap-identity-provider[Configuring LDAP identity provider, window=blank]
...
* **Group Sync:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication_and_authorization/ldap-syncing[Syncing LDAP groups, window=blank]
...
----
oc login -u {openshift_cluster_admin_username} -p {openshift_cluster_admin_password} --insecure-skip-tls-verify
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/cleanup-ldap.sh {support_base_url}/cleanup-scripts/cleanup-ldap.sh; then
  bash /tmp/cleanup-ldap.sh
elif curl -fsSL --retry 3 --retry-delay 2 -o /tmp/cleanup-ldap.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/cleanup-scripts/cleanup-ldap.sh; then
  bash /tmp/cleanup-ldap.sh
else
  echo "ERROR: Could not download cleanup-ldap.sh from the Showroom instance or GitHub. Check your network connection and re-run this command."
fi
----
