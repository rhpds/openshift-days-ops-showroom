...
until oc login -u normaluser1 -p 'Op#nSh1ft' $(oc whoami --show-server) --insecure-skip-tls-verify 2>&1 | grep -q "Login successful"; do
  sleep 10
done
oc login -u admin --insecure-skip-tls-verify 2>/dev/null
