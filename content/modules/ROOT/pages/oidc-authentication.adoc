...
NOTE: This lab uses Red Hat Single Sign-On (RHSSO). For new deployments, consider the **Red Hat build of Keycloak (RHBK)** which is the successor product with a Quarkus-based architecture and improved performance.
...
[source,bash,role="execute"]
----
echo "https://$(oc get route keycloak -n rhsso -o jsonpath='{.spec.host}')/auth/admin/"
----

Log in with the Keycloak admin credentials:

[source,bash,role="execute"]
----
echo "Username: admin"
echo "Password: $(oc get secret credential-rhsso -n rhsso -o jsonpath='{.data.ADMIN_PASSWORD}' | base64 -d)"
----

In the Keycloak admin console, select the **OpenShift** realm from the dropdown (top-left). Click **Users**, then **View all users** to see the three accounts:
...
OIDC typically uses the **authorization code flow** which requires a web browser for authentication. Unlike LDAP, you generally cannot use `oc login -u username -p password` directly with OIDC unless your provider supports the Resource Owner Password Credentials (ROPC) grant flow.
...
Verify the Keycloak client has the groups protocol mapper configured (created by the setup script in Step 3).
