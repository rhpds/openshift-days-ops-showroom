...
1. In the **OCP Console** tab, click on your username in the top-right corner and select **Log out**:
+
image::oidc-logout-menu.png[OpenShift console logout menu]
+
2. You'll see the login options — click **rhbk**
3. You'll be redirected to the Keycloak login page. Log in with: `developer1` / `OpenShift123!`
+
image::oidc-keycloak-login.png[Keycloak login page]
+
4. You'll be redirected back to the OpenShift console, logged in as Developer User:
+
image::oidc-logged-in-developer.png[OpenShift console logged in as Developer User via OIDC]

[NOTE]
====
You may see an error message about permissions. This is **expected** — `developer1` is a regular user without cluster-admin. You'll configure RBAC in Step 7. To fix the console view, click the **reload button** (circular arrow) next to the OCP Console tab to refresh the session, then click the **rhbk** tab again if needed.
====
