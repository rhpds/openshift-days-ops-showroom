...
In the Keycloak admin console, select the **OpenShift** realm from the dropdown (top-left). Click **Users** — you'll see admin1, developer1, viewer1. Click **Groups** — you'll see ocp-admins, ocp-developers, ocp-viewers. Click **Clients** -> **openshift** -> **Mappers** — you'll see the `groups` protocol mapper that puts group membership into the ID token. This mapper is the key to automatic group sync — without it, OpenShift wouldn't know what groups a user belongs to.
