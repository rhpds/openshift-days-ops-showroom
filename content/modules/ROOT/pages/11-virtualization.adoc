...
Log in with the credentials above, then run these commands inside the VM to make changes we can verify after restore:
...
NOTE: You may see boot output (kernel messages, systemd services) scrolling. Wait for the `rhel9-vm login:` prompt to appear, then press **Enter** if needed.
