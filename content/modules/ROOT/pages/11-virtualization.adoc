...
+
image::virt-create-dropdown.png[Create VirtualMachine dropdown showing From InstanceType option, link=self, window=blank]. Verify the **Project** dropdown at the top shows `vm-demo` - if not, search for it and select it now
+
...
+
image::virt-actions-migrate.png[Actions dropdown showing Migration submenu with Compute and Storage options, link=self, window=blank]. In the migration dialog, leave **Automatically selected Node** selected and click **Migrate VirtualMachine**
+
...
+
image::virt-actions-clone.png[Actions dropdown with Clone option highlighted, link=self, window=blank]. Change the name to `rhel9-vm-clone`
...
+
image::virt-actions-snapshot.png[Actions dropdown with Take snapshot option, link=self, window=blank]. Leave the default name (`rhel9-vm-snapshot`) and click **Save**
+
...
+
image::virt-snapshot-restore-menu.png[Snapshot kebab menu showing Restore VirtualMachine from snapshot option, link=self, window=blank]. Click **Restore** to confirm
+
image::virt-restore-confirmation.png[Restore snapshot confirmation dialog, link=self, window=blank]. **Start** the VM again by clicking the **Start** button (play icon)
