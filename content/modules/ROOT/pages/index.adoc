...
3. The **Overview** tab shows you the node's health at a glance:
   - **Status** - at the top you'll see the overall node status and any key issues. On this workshop cluster you'll likely see warnings about CPU or memory overcommit — this means the total **limits** of all pods exceed the node's capacity. Look at the **Utilization** section below to see that actual usage is well within capacity. The difference is important: limits are what pods are _allowed_ to burst to, not what they actually use.
   - **Alerts** - below the status box you'll see firing alerts like `KubeCPUOvercommit`, `PodDisruptionBudgetAtLimit`, and others. On a workshop cluster with many operators installed, this is normal.
   - **Utilization** - CPU, Memory, Filesystem, and Network over time. Compare "available" to "total limit" — that tells you the real capacity story.
...
Status, alerts, utilization, and per-pod resource usage — all in one console view, no SSH to nodes required.
