...
First, a quick word on `oc`: it's the OpenShift CLI - like `kubectl`, but with extra OpenShift-specific commands (Routes, Projects, and more). Every `oc` command below runs in the **Terminal** tab, a web-based shell already connected and authenticated to your cluster - no local install or login required. Clicking a highlighted command block automatically switches to that tab and runs it for you.
