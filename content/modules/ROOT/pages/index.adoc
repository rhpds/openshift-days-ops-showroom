...
NOTE: This workshop uses cluster-admin throughout so you can explore platform-wide features (nodes, MachineSets, cluster operators) without hitting permission walls. In production, most day-2 application tasks - deploying, scaling, checking logs - only need project/namespace-admin. Cluster-admin is generally reserved for platform-level work.

NOTE: `oc` is the OpenShift CLI - like `kubectl`, but with extra OpenShift-specific commands (Routes, Projects, and more). Every `oc` command below runs in the **Terminal** tab, which is a web-based shell already connected and authenticated to your cluster - no local install or login required. Clicking a highlighted command block automatically switches to that tab and runs it for you.
...
=== Step 1: Access the Core Platform Console

The OpenShift Web Console provides a unified view of your entire cluster.

image::ocp-console-overview.png[OpenShift Core Platform Console Overview, link=self, window=blank]
...
=== Step 2: Switch to Core Platform Perspective

In the top-left corner of the console, you'll see a perspective switcher dropdown. Click the dropdown and select **Core Platform**.

=== Step 3: Dashboard Overview

Once in the Core Platform perspective, you'll see the **Overview** dashboard under **Home -> Overview**.

image::ocp-dashboard-overview.png[OpenShift Core Platform Dashboard Overview, link=self, window=blank]
...
control-plane-cluster-xxxxx-1   Ready    control-plane,master,worker   20h   v1.x.x
control-plane-cluster-xxxxx-2   Ready    control-plane,master,worker   20h   v1.x.x
control-plane-cluster-xxxxx-3   Ready    control-plane,master,worker   20h   v1.x.x
----
...
NOTE: When Advanced Cluster Management (ACM) is installed, `machinesets` exists in two API groups (Cluster API and OpenShift Machine API). We use the full resource name `machinesets.machine.openshift.io` to target OpenShift's Machine API.
...
3. You'll see the unified software catalog with categories on the left:
   ...
   - **Streaming & Messaging** - Red Hat AMQ Streams (Kafka)
