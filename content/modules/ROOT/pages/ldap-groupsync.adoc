...
curl -sL https://certs.godaddy.com/repository/gd-class2-root.crt -o $HOME/support/ca.crt
----
...
[NOTE]
====
The OAuth operator rollout can take 2-3 minutes to complete. If you get an error like `error: invalid credentials` or `unexpected response status 500` when trying to log in, wait a few minutes and try again.
====
...
Result: You may see some system namespaces but **no application projects** (app-dev, app-test, app-prod). This user is not in any project-specific groups.
...
# Remove cluster-admin from fancyuser1
oc adm policy remove-cluster-role-from-user cluster-admin fancyuser1 --ignore-not-found
oc adm policy remove-cluster-role-from-group cluster-reader ose-fancy-dev --ignore-not-found
...
oc rollout status deployment/oauth-openshift -n openshift-authentication
echo "Cleanup complete — OAuth restored to htpasswd only"
----
