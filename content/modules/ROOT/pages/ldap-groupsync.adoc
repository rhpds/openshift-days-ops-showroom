...
In the **OCP Console**, navigate to **User Management -> RoleBindings**, select project `app-dev`, and click the rolebinding named **edit**. Switch to the **YAML** tab to see the full binding — it shows the group `ose-teamed-app` bound to the `edit` ClusterRole:

image::ldap-rolebinding-yaml.png[RoleBinding YAML showing ose-teamed-app group bound to edit ClusterRole in app-dev]
