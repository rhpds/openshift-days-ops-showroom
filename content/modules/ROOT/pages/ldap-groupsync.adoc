...
**See this in the console too:** Navigate to **User Management -> Groups** in the **OCP Console**. Click on `ose-fancy-dev` — you'll see fancyuser1 and fancyuser2 listed as members. Now click **User Management -> Users** — notice only `normaluser1` appears (and `admin`). The other users exist in LDAP groups but haven't logged in yet, so OpenShift hasn't created their User objects.
