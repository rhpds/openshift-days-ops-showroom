...
[NOTE]
====
In a fresh cluster, `spec` would be empty (`spec: {}`). This workshop cluster has HTPasswd pre-configured. When you add LDAP below, it will be added alongside the existing HTPasswd provider so both work simultaneously.
====
