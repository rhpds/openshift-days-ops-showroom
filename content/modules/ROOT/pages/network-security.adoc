...
OpenShift uses **OVN-Kubernetes** as the default network plugin. OVN-Kubernetes became the default for new installations in OpenShift 4.12 and is the only supported network plugin in OpenShift 4.20.
...
Now switch to the **OCP Console** tab and navigate to **Networking -> NetworkPolicies** (project: `netpol-backend`). Click on `allow-from-frontend` — the console renders the ingress rules as a visual diagram: which namespaces can reach which pods on which ports. Compare this to the YAML you just wrote. In production, when you're auditing policies across 50 namespaces, this view is dramatically faster than reading raw YAML. Click `deny-all` too — notice how clearly it shows "no ingress allowed."
...
[source,bash,role="execute"]
----
oc api-resources | grep adminnetworkpolicy
----
...
You can also use DNS names (OVN resolves them). Wildcard DNS name resolution in EgressFirewall rules is a Technology Preview feature in OpenShift 4.20:
