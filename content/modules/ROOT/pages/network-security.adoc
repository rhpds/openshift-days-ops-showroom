...
TIP: In the **OCP Console**, navigate to **Networking -> NetworkPolicies** (project: `netpol-backend`). Click on any policy to see the ingress/egress rules rendered visually — peer selectors and allowed ports are displayed as readable rules rather than nested YAML. This is useful for auditing security policies across namespaces.
