...
* `spec.ols.introspectionEnabled` - When set to `true`, OpenShift Lightspeed automatically deploys an MCP server that allows users to query cluster resources to provide context-aware responses (Technology Preview in OpenShift 4.22)
...
ELAPSED=0
until oc get pods -n openshift-lightspeed -l app.kubernetes.io/component=application-server --no-headers 2>/dev/null | grep -q "2/2.*Running"; do
...
lightspeed-app-server-xxxxx                              2/2     Running   1               3m
...
lightspeed-rhokp-xxxxx                                   1/1     Running   0               3m
openshift-mcp-server-xxxxx                               1/1     Running   0               3m
----

NOTE: The postgres and app-server pods may each show 1 restart - this is normal during initial startup.
...
Under the hood, OpenShift Lightspeed searches the official OpenShift documentation using the Offline Knowledge Portal (OKP), which replaced the product's earlier custom RAG implementation. OKP uses tool-based retrieval - the model decides during its reasoning whether to invoke the documentation-search tool, rather than every response always attaching a fixed list of source links.

OpenShift Lightspeed will likely provide a response explaining what both Deployment and StatefulSet resources are, along with a comparison between them.

NOTE: Because OKP uses tool-based retrieval, the model decides per-question whether to invoke the documentation-search tool - so you won't always see a list of linked source documents at the end of the response, even for accurate answers.
== Capabilities
...
Now, in the chat window, click the *+* button next to the typing bar and you will see a menu that allows you to attach different components. Select *Full YAML file*:
