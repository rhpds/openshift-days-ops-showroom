...
[source,bash,role="execute"]
----
oc get pvc test-pvc && oc get pod test-storage
----

The PVC should show `Bound` and the pod should be `Running`. Check the pod wrote to the volume successfully:

[source,bash,role="execute"]
----
oc logs test-storage
----
...
[source,bash,role="execute"]
----
oc delete pod test-storage && oc delete pvc test-pvc
----
...
The best way to verify monitoring is working is to actually use it. In the **OCP Console**, navigate to **Observe -> Dashboards** and select the **etcd** dashboard. Find the **Raft proposals** and **leader changes** panels. In a healthy cluster, leader changes should be 0 or very low. Frequent leader elections indicate disk I/O problems or network latency between control plane nodes — the kind of issue that causes cascading API failures at 2am. This dashboard is the first place to look when `oc` commands start timing out.
