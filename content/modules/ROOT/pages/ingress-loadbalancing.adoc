...
You can also view these in the **OCP Console** at **Workloads -> Pods** (project: `openshift-ingress`):
...
image::ingress-images/ingress-route-annotations.png[Route YAML showing rate-limit annotations]
...
image::ingress-images/ingress-service-pods.png[Service weathernow showing 3 backend pods]
...
Scale to a different replica count to see the behaviour. Let's scale down to 2 and then back:
...
Press `Ctrl+C` when you see 2 pods running. Now scale back:
