...
Now let's examine its networking.
...
TIP: In the **OCP Console**, navigate to **Networking -> Routes** (select **All Projects**). The console shows each route with its TLS termination type as a badge, the target service, and a clickable URL — you can test routes directly from the console without needing `curl`.
