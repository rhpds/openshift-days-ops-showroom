...
OpenShift uses **OVN-Kubernetes** as its network plugin (the default network plugin). It handles pod networking, network isolation via NetworkPolicy, egress traffic control, and optional IPsec encryption.
...
TIP: This isn't just a best practice - OpenShift uses the same pattern to protect its own platform namespaces. Check it: `oc get networkpolicies -n openshift-dns` - you'll see `openshift-dns-deny-all` already in place, with a second policy that allows only the traffic DNS needs. Every platform namespace ships with default-deny policies out of the box.
...
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/test-dns.sh {support_base_url}/04-network-security/test-dns.sh; then
  bash /tmp/test-dns.sh
elif curl -fsSL --retry 3 --retry-delay 2 -o /tmp/test-dns.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/04-network-security/test-dns.sh; then
  bash /tmp/test-dns.sh
else
  echo "ERROR: Could not download test-dns.sh from the Showroom instance or GitHub. Check your network connection and re-run this command."
fi
----
...
NOTE: Wildcard DNS names (e.g., `*.redhat.com`) are a Technology Preview feature in OpenShift 4.22 and may not be enabled on all clusters. Use specific hostnames for production configurations.
...
Nodes must have the label `k8s.ovn.org/egress-assignable=""` to be eligible to host EgressIPs.
...
* **OVN-Kubernetes:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/ovn-kubernetes_network_plugin/index[OVN-Kubernetes network plugin, window=blank]
* **NetworkPolicy:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/network_security/network-policy[About network policy, window=blank]
* **EgressFirewall:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/network_security/egress-firewall[Configuring an egress firewall for a project, window=blank]
* **EgressIP:** link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/ovn-kubernetes_network_plugin/configuring-egress-ips-ovn[Configuring an egress IP address, window=blank]
