...
[source,bash,role="execute"]
----
if curl -fsSL --retry 3 --retry-delay 2 -o /tmp/test-dns.sh https://raw.githubusercontent.com/rhpds/openshift-days-ops-showroom/dev/support/04-network-security/test-dns.sh; then
  bash /tmp/test-dns.sh
else
  echo "ERROR: Could not reach GitHub to download test-dns.sh. Check your network connection and re-run this command - GitHub may be experiencing a temporary outage."
fi
----
