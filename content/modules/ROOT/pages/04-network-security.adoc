...
TIP: You can also view this in the console at **Administration > Cluster Settings > Configuration > Network** for a formatted view of cluster and service network CIDRs.
...
image::netpol-deny-all.png[deny-all NetworkPolicy showing all incoming traffic denied]
...
image::netpol-allow-frontend.png[allow-from-frontend NetworkPolicy showing selectors and port rule]
...
[source,bash,role="execute"]
----
oc exec -n netpol-backend deployment/backend -- nslookup -timeout=2 -retry=1 google.com 8.8.8.8
----

Expected: `communications error to 8.8.8.8#53: timed out` followed by `no servers could be reached`. The ANP blocked outbound DNS to 8.8.8.8.
...
Expected: DNS resolution succeeds again - the ANP is gone.
...
You should see an `Address:` line with the backend service's cluster IP - this confirms that pods can resolve other services by name. Ignore the `Got recursion not available` messages, they're harmless (CoreDNS disables recursion by default).
