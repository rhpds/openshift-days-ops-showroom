...
OpenShift GitOps uses link:https://argo-cd.readthedocs.io/en/stable/[ArgoCD] to implement a continuous reconciliation loop:
...
For more details, see link:https://docs.redhat.com/en/documentation/red_hat_openshift_gitops/1.19/html/understanding_openshift_gitops/index[Understanding OpenShift GitOps].
...
Authorise access when prompted. You should now see the empty ArgoCD dashboard.
...
Switch to the ArgoCD UI. You should see the `managed-project` application tile appear on the dashboard.
...
* `Application: managed-project` at the top
* Branching down to: `Namespace`, `ResourceQuota`, `LimitRange`, `NetworkPolicy`

Each resource node shows its sync and health status with colour-coded icons. Click on any resource — for example, the **ResourceQuota** — to open a detail panel on the right. This panel shows:

* **Summary** — kind, namespace, creation time
* **Live Manifest** — the current state on the cluster (what's actually running)
* **Desired Manifest** — what's defined in Git (what it should be)
...
Click **Back** or the ArgoCD logo to return to the dashboard.

=== Explore the Application Details

From the application view, click the **App Details** button in the top left. This shows the application configuration:

* **Source** — the Git repository URL, branch, and path ArgoCD is watching
* **Destination** — which cluster and namespace the resources are deployed to
* **Sync Policy** — whether sync is automatic or manual, and whether self-heal and prune are enabled
...
TIP: In the OpenShift console, navigate to **Home -> Projects** and select `gitops-managed-project`. You can see the ResourceQuota and LimitRange under the project details.
...
Go back to the ArgoCD UI dashboard. The `managed-project` application tile now shows a **yellow warning icon** with **OutOfSync** status. ArgoCD detected that the live cluster state no longer matches what's defined in Git.

Click the tile to open the application. In the resource tree, the **ResourceQuota** node is highlighted in yellow — that's the resource that drifted.

**View the diff:**

Click on the `compute-quota` ResourceQuota in the resource tree. In the detail panel on the right, click the **Diff** tab. You'll see a side-by-side comparison:

* **Green** — the value from Git (`pods: "20"`) — this is the desired state
* **Red** — the manual change (`pods: "100"`) — this is what's actually on the cluster
...
In the ArgoCD application view, click the **Sync** button in the top toolbar, then click **Synchronize** in the dialog that appears. You can leave all defaults — ArgoCD will apply every resource from Git.

Watch the resource tree — the ResourceQuota node will flash as it updates, then return to green (Synced). The application tile on the dashboard will also return to a green Synced status.
...
[source,bash,role="execute"]
----
oc patch application managed-project -n openshift-gitops \
...
You can also click on the `compute-quota` ResourceQuota and look at the **Events** tab to see the sync history — you'll see ArgoCD detected the drift and automatically resynced.
...
[source,bash,role="execute"]
----
oc delete application managed-project -n openshift-gitops --ignore-not-found
