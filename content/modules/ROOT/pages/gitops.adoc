...
Look for the grid icon (nine tiles) in the top navigation bar of the OpenShift console and select **Cluster Argo CD** under OpenShift GitOps:

image::gitops-nine-tiles.png[OpenShift console grid menu showing Cluster Argo CD link]
...
image::gitops-argocd-login.png[ArgoCD login page with Log in via OpenShift button]
...
image::gitops-openshift-login.png[OpenShift login form]

Authorise access when prompted:

image::gitops-authorize-access.png[OAuth authorize access page]

TIP: If you have the **ArgoCD** tab in Showroom, you can use that instead — it's already authenticated.
...
Switch to the ArgoCD UI. You may see other applications on the dashboard — these are managed by the platform. To see only your workshop application, use the **PROJECTS** filter on the left sidebar and select **workshop**:

image::gitops-project-filter.png[ArgoCD PROJECTS filter dropdown showing default and workshop options]

You should now see only the `managed-project` application tile:

image::gitops-dashboard-synced.png[ArgoCD dashboard filtered by workshop project showing managed-project as Healthy and Synced]
...
image::gitops-resource-tree-synced.png[ArgoCD resource tree showing managed-project with four child resources all Synced]
...
image::gitops-quota-detail.png[ArgoCD detail panel for compute-quota showing Summary and Live Manifest tabs]
...
image::gitops-app-details.png[ArgoCD App Details showing project workshop, repo URL, path, and sync status]
...
image::gitops-resource-tree-outofsync.png[ArgoCD resource tree showing OutOfSync status with compute-quota highlighted]

**View the diff:**

Click on the `compute-quota` ResourceQuota in the resource tree. In the detail panel, click the **Diff** tab:

image::gitops-quota-outofsync.png[compute-quota detail showing OutOfSync status]

You'll see a side-by-side comparison showing the exact difference:

image::gitops-diff-view.png[ArgoCD diff view showing pods 100 on the left vs pods 20 on the right]
...
image::gitops-sync-dialog.png[ArgoCD sync dialog showing all four resources checked for synchronization]
...
image::gitops-resynced.png[ArgoCD resource tree back to all Synced after resync]
...
image::gitops-selfheal-synced.png[ArgoCD dashboard showing managed-project still Synced after self-heal]
