...
You'll see CPU usage broken down by namespace. Look at which namespaces are consuming the most — `openshift-monitoring` and `openshift-etcd` are usually at the top because they run the cluster's own infrastructure. Everything you're seeing here was available the moment the cluster came up — no dashboards to build, no scrape configs to write, no Grafana to install.
...
Let's prove the point. Deploy a web server and make it externally accessible — all in three commands:
...
You should see the httpd test page HTML. That's a deployed app, with a service, a TLS-terminated route, and automatic DNS — in three commands. In vanilla Kubernetes, you'd need to install an ingress controller, create an Ingress resource, configure DNS records, and manage TLS certificates.
...
2. Click on any node
3. Scroll down to the **Conditions** section. Look for `MemoryPressure`, `DiskPressure`, and `PIDPressure` — all should be `False`. If any of these flip to `True`, OpenShift starts evicting pods from that node. This is the first place to check when pods are being mysteriously killed.
4. Check the **Pods** tab to see every pod running on that node, sorted by resource usage.

In vanilla Kubernetes, you'd SSH to the node and run `top`, `df`, and `crictl pods`. In OpenShift, it's all in one console view.
...
**What happens:** OpenShift automatically provisions a compute instance, configures it, and joins it to the cluster.
...
* **Installation & Verification** - Checking cluster health
* **Application Management** - Deploying and scaling applications
* **Networking** - Ingress, load balancing, and network security
* **Day 2 Operations** - Debugging, observability, and backup/restore
