...
While the operators install, navigate to **Ecosystem -> Installed Operators** in the **OCP Console** (select **All Projects**) and watch them appear. You'll see the status change from `Installing` to `Succeeded` in real-time — this is how you'd monitor operator installs in production.

image::observe-operator-install.png[Installed Operators showing Loki Cluster Logging and Cluster Observability operators with Succeeded status]

Wait for all three operators to be ready (this can take 2-3 minutes):
...
All operators should show `Succeeded`. **Do not proceed to the next step until all three show Succeeded** — the LokiStack and UIPlugin resources require the operator CRDs to be installed first.
