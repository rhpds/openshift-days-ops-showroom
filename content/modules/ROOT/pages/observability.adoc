...
Remove the logging stack you deployed. The Prometheus monitoring stack is built-in and doesn't need cleanup.

[source,bash,role="execute"]
----
# Remove the logging pipeline and UI
oc delete clusterlogforwarder collector -n openshift-logging --ignore-not-found
oc delete uiplugin logging --ignore-not-found

# Remove LokiStack and storage
