...
Once the UIPlugin is active, navigate to **Observe -> Logs** in the console. You'll see three log types:

* **Application** — Logs from user workloads
* **Infrastructure** — Logs from OpenShift components
* **Audit** — Kubernetes API audit logs

Use the filters to search by namespace, pod name, or severity level. The histogram at the top shows log volume over time.
