...
image::debugging-frontend-events.png[weather-frontend pod Events tab showing ImagePullBackOff error]
...
image::debugging-api-logs.png[weather-api pod Logs tab showing missing config file error]
...
image::debugging-cache-deployment-events.png[weather-cache deployment Events showing FailedCreate due to LimitRange violation]
...
TIP: You can verify the ConfigMap in the console at **Workloads > ConfigMaps** (project: `ops-track-demo`) — click `weather-config` to see its keys and values.
...
image::debugging-proxy-service-no-pods.png[weather-proxy Service showing 0 Pods due to selector mismatch]
...
image::debugging-all-fixed.png[All five weather pods showing green Running status]
...
NOTE: If the **Developer Hub** module is part of your workshop, **skip this cleanup** — it uses the weather app pods in `ops-track-demo` to demonstrate the Kubernetes plugin.

[source,bash,role="execute"]
----
oc delete namespace ops-track-demo --ignore-not-found
